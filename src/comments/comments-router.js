...
commentsRouter.route('/').get((req, res, next) => {
    ...
    CommentsService.getAllComments(knexInstance).then(comments => {
        res.json(comments.map(serializeComment))
    })
    .catch(next)
})
.post(jsonParser, (req, res, next) => {
    const { test, article_id, user_id, date_commented } = req.body
    const newComment = { test, article_id, user_id }
...
.patch(jsonParser, (req, res, next) => {
    ...
    const numberOfValues = Objects.values(commentToUpdate).filter(Boolean).length
    if (numberOfValues === 0)
    return res.status(400).json({
        error: {
